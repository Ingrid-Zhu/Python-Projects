...
class Filter(a6editor.Editor):
    ...
    def invert(self):
        """
        Inverts the current image, replacing each element with its
        color complement
        """
    ...
    def monochromify(self, sepia):
        ...
        current=self.getCurrent()
        if sepia == False:
            for pos in range(current.__len__()):
                rgb = current[pos]
                brightness=0.3*rgb[0]+0.6*rgb[1]+0.1*rgb[2]
                rgb = (int(brightness), int(brightness),
                       int(brightness))
                current[pos] = rgb
        else:
            for pos in range(current.__len__()):
    ...
    def jail(self):
        ...
        * Put n 4-pixel vertical bars inside, where n is
        ...
        d = (current.getWidth()-8-(n*4))/(n+1)
        for i in range(n):
            self._drawVBar(int(round((i+1)*(4+d))),(255,0,0))
    ...
    def vignette(self):
        ...
        cc=height/2
        cr=width/2
        h =math.sqrt((height-cr)**2 + (width-cc)**2)
        for row in range(current.getHeight()):
            for col in range(current.getWidth()):
                rgb =current.getPixel(row,col)
                d=math.sqrt((row-cr)**2 + (col-cc)**2)
                f=1-(d**2)/(h**2)
                red=rgb[0]*f
                green=rgb[1]*f
                blue=rgb[2]*f
